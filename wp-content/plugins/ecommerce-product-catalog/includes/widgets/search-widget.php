...
class product_widget_search extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			...
			$label		 = sprintf( __( '%s Search', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
			$sublabel	 = sprintf( __( 'A search form for your %s.', 'ecommerce-product-catalog' ), lcfirst( $names[ 'plural' ] ) );
		} else {
