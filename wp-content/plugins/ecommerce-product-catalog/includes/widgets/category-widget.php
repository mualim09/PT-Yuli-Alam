...
class product_cat_widget extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			...
			$label		 = sprintf( __( '%s Categories', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
			$sublabel	 = sprintf( __( 'A list or dropdown of %s categories', 'ecommerce-product-catalog' ), lcfirst( $names[ 'singular' ] ) );
		} else {
	...
	function widget( $args, $instance ) {
		if ( get_integration_type() != 'simple' ) {
			...
			if ( $d ) {
				if ( is_plural_form_active() ) {
					$names	 = get_catalog_names();
					$label	 = sprintf( __( 'Select %s Category', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
				} else {
