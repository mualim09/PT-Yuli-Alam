...
function register_product_filter_bar() {
	if ( is_plural_form_active() ) {
		...
		$label		 = sprintf( __( '%s Filters Bar', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
		$sublabel	 = sprintf( __( 'Appears above the product list. Recommended widgets: %1$s Search, %1$s Price Filter, %1$s Sort and %1$s Category Filter.', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
	} else {
...
class product_category_filter extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			$names		 = get_catalog_names();
			$label		 = sprintf( __( '%s Category Filter', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
			$sublabel	 = sprintf( __( 'Filter %s by categories.', 'ecommerce-product-catalog' ), lcfirst( $names[ 'plural' ] ) );
		} else {
...
class product_sort_filter extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			$names		 = get_catalog_names();
			$label		 = sprintf( __( '%s Sort', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
			$sublabel	 = sprintf( __( 'Sort %s dropdown.', 'ecommerce-product-catalog' ), lcfirst( $names[ 'plural' ] ) );
		} else {
...
class product_price_filter extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			$names		 = get_catalog_names();
			$label		 = sprintf( __( '%s Price Filter', 'ecommerce-product-catalog' ), ucfirst( $names[ 'singular' ] ) );
			$sublabel	 = sprintf( __( 'Filter %s by price.', 'ecommerce-product-catalog' ), lcfirst( $names[ 'plural' ] ) );
		} else {
